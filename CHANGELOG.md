...
| `conformance/runner/example-harness/{sdk,harness}.mjs` | The reference SDK posts to v2 and sends all five §7.2 REQUIRED headers on every request — `api-key`, `env`, `X-Avo-Client`, `Content-Type: application/json` and `Content-Length` (set from the payload actually sent, so a gzipped body carries the compressed length) — including when the body is gzipped and when the mock endpoint overrides the URL; it implements §4.2.1 / §7.3.6, reads the renamed `options.originAppVersion`, and no longer sends `sessionId`. It also implements the new control-character rule on both sides: the constructor throws the exact §4.1 message on an `apiKey` containing CR, LF or NUL, and `_send` refuses to transmit any header value carrying one, dropping and logging the batch (header name only, §7.5.1) rather than stripping or substituting. Without this the reference SDK demonstrated behaviour the spec forbids, which matters because generators read it as the worked example. No fixture covers either check, for the reason recorded in the coverage map. `npm run conformance:run` is green at 36/36. |
...
| `scripts/check-reference-sdk.mjs` (new), `package.json` | **The control-character rule had no regression coverage.** It was stated in SPEC.md §4.1 and §7.2 and implemented in the reference SDK, and deleting either guard left the suite green at 36/36 — the same condition that let the reference SDK ship without the rule in the first place. A fixture cannot close it: "zero requests captured" passes for an SDK that refused the send, one that never validated and whose HTTP client happened to reject the value, and one that threw at construction. The new check runs in process and replaces `fetch` with a probe, which distinguishes those, and asserts the inverse too — a clean `apiKey` must still reach the wire, since a guard refusing every send would satisfy every other assertion. Proven to bite in all three directions: removing the constructor guard fails 4 checks, removing the send guard fails 2, and making the guard refuse everything fails the 2 controls. Exposed as `npm run check:reference-sdk`, with a new `npm test` running fixtures, this check and the conformance suite in order. It gates this repository's reference SDK only; a generated SDK still needs its own equivalent, which the coverage map now says explicitly. |
| `scripts/validate-fixtures.mjs` | The script validated only `eventProperties[]` elements and never the event bodies containing them, so `schemas/event-body.json`'s `required` array, its forbidden `not.anyOf` and every base field's type were unexercised by the repo's own tooling — a fixture that dropped or mistyped a base field passed clean. It had been left that way because the bodies carry placeholder markers no strict schema accepts. It now substitutes the weakest value satisfying each marker's runtime predicate, drops keys marked `<absent>` (which assert the key is off the wire), and validates the body around them; the four placeholder values themselves stay the suite runner's job, which is stated in the script and its spec so nobody assumes wider coverage. Forbidden-field failures name the field, since Ajv reports a failed `not` only as "data must NOT be valid". This mattered here because 3.0.0 edits 26 fixture bodies to remove `sessionId`, and a dropped base field is exactly the error such a sweep introduces. |
