...
| `conformance/runner-contract.md` (1.1.0) | `AVO_INSPECTOR_MOCK_ENDPOINT` points at v2 and replaces the URL only; new normative "Required request headers" section (asserted on every captured request); placeholders are accepted as `expected_request_headers` values; recorded-headers example and the implementation checklist updated. `expected_request_headers` is documented as available in **every** fixture mode rather than only the wire-protocol suite, and the sequence-mode assertion table now lists it — `batch-1` is a sequence fixture that uses it, so the field could not stay documented as wire-protocol-only. The stdin/stdout envelope is unchanged — an existing harness needs no edit, only the SDK it drives. |
